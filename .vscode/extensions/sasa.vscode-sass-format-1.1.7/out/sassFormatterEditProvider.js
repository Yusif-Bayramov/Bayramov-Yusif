...
exports.SassFormatterEditProvider = void 0;
...
class SassFormatterEditProvider {
    ...
    _getTextEdit(document, range) {
        const optionFormatOnPaste = vscode_1.workspace
        ...
        if (range === undefined || optionFormatOnPaste) {
            ...
            let rangeEnd = document.lineAt(document.lineCount - 1)
                .range.end;
    ...
    _formatSass(text, extName) {
        const optionUseSingleQuotes = vscode_1.workspace
            .getConfiguration("sassFormat")
            .get("useSingleQuotes");
        const optionInlineComments = vscode_1.workspace
            .getConfiguration("sassFormat")
            .get("inlineComments");
        const optionNumberLeadingZero = vscode_1.workspace
            .getConfiguration("sassFormat")
            .get("numberLeadingZero");
        let result;
        ...
        try {
            ...
            result = child_process_1.execSync(sassConvertFormatCommand, {
                encoding: "utf8",
                input: text
            });
            if (optionInlineComments) {
    ...
    _getSassConvertOptions(extName) {
        const optionDasherize = vscode_1.workspace
            .getConfiguration("sassFormat")
            .get("dasherize");
        const optionIndent = vscode_1.workspace
            .getConfiguration("sassFormat")
            .get("indent");
        // const optionOldStyle = workspace.getConfiguration('sassFormat').get<boolean>("oldStyle"); // deprecated
        const optionDefaultEncoding = vscode_1.workspace
            .getConfiguration("sassFormat")
            .get("defaultEncoding");
        const optionUnixNewlines = vscode_1.workspace
            .getConfiguration("sassFormat")
            .get("unixNewlines");
